...
def main():
    print("\n--- Impact of Max Sequence Length ---")
    print("Fixed Depth = 4, Batch Size = 16")
    print("Seq Len\tParams (M)\tModel VRAM (MB)\tPeak Training VRAM (MB)")
    print("-" * 65)
    ...
    for seq_len in seq_lens:
        params, model_mem, peak_vram = profile_model(depth, seq_len, batch_size)
        vram_str = f"{peak_vram:.1f}" if peak_vram > 0 else "OOM"
        print(f"{seq_len}\t{params/1e6:.1f}\t\t{model_mem:.1f}\t\t{vram_str}")


    print("\n--- Impact of Batch Size ---")
    print("Fixed Depth = 4, Seq Len = 256")
    print("Batch Size\tParams (M)\tModel VRAM (MB)\tPeak Training VRAM (MB)")
    print("-" * 65)
    ...
    for bs in batch_sizes:
        params, model_mem, peak_vram = profile_model(depth, seq_len, bs)
        vram_str = f"{peak_vram:.1f}" if peak_vram > 0 else "OOM"
        print(f"{bs}\t\t{params/1e6:.1f}\t\t{model_mem:.1f}\t\t{vram_str}")
